	...
	<ui-layout class="wallet-wrap" navTitle="钱包">
		...
		<view class="header-box ss-flex ss-row-center ss-col-center ">
			<view class="card-box ui-BG-Main ui-Shadow-Main">
				...
				<view class="money-num ss-p-t-50">{{ state.showMoney ? userInfo.money : '*****' }}</view>
		...
		<!-- 钱包记录 -->
		<view class="wallet-list ss-flex border-bottom" v-for="item in state.list" :key="item.id">
...
const getList = async () => {
	let res = await sheep.$apis.user.wallet.log({ type: "money" });
	if (res.error === 0) {
		state.list = _.concat(state.list, res.data.list.data);
	}
};
onBeforeMount(async () => {
	getToday();

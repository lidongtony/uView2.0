<template>
	<ui-layout class="wallet-wrap" navTitle="佣金">
		<!-- 佣金卡片 -->
		<view class="header-box ss-flex ss-row-center ss-col-center ">
			<view class="card-box ui-BG-Main ui-Shadow-Main">
				...
				<view class="btn-box ss-flex">
					<button class="ss-reset-button withdraw-btn"
						@tap="sheep.$router.go('/pages/user/wallet/withdraw')">提现</button>
					<button class="ss-reset-button balance-btn ss-m-l-20"
						@tap="transfterBalanceRef.showModal()">转余额</button>
				</view>
		...
		<ui-fixed bg="bg-white" sticky :index="100">
			...
			<view class="wallet-tab-card ss-flex ss-col-center border-bottom">
				<view class="tab-item ss-flex-1 ss-flex ss-row-center ss-col-center" v-for="item in state.tabList"
					...
					<view class="tab-title" :class="[{'cur-tab-title':state.curTab === item.value}]">
						{{item.label}}
					</view>
		...
		<view class="wallet-list ss-flex border-bottom" v-for="item in 10" :key="item">
			<image class="head-img ss-m-r-20" src="/static/images/tabbar/category2.png" mode="aspectFit"></image>
		...
		<!-- 转余额 -->
		<transfter-balance ref="transfterBalanceRef"></transfter-balance>
	</ui-layout>
</template>
...
<script setup>
	...
	import transfterBalance from './components/transfter-balance.vue'
	...
	// 数据
	const transfterBalanceRef = ref(null)
